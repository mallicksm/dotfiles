...
***************************************************************************/
static const char svnRevisionString[] = "$LastChangedRevision: 149764 $";
...
int T32_GetChannelSize(void)
{
	...
	size = sizeof(Channel_t) + gT32InternalLineDriverNetAssist->GetParamsSize();
...
void T32_GetChannelDefaults(void *pVoid)
{
	...
	pChannel->pLineDriver = gT32InternalLineDriverNetAssist;
	pChannel->pLineParams = (void*)(pChannel + 1);
	gT32InternalLineDriverNetAssist->DefaultParams((LineStruct *)(pChannel->pLineParams));
...
int T32_RequestChannelNetAssist(void **ppVoid)
{
	...
	pChannel->pLineDriver = gT32InternalLineDriverNetAssist;
	pChannel->pLineParams = malloc(gT32InternalLineDriverNetAssist->GetParamsSize());
	...
	int nInfoID, T32_Param * value)
{
	...
	if (!info64) {
		...
		if (Handle > T32_TAPACCESS_HOLD)
			res = T32_BundledAccessStore(0, Handle, size * 8, outbits, (uint8_t*)(void*)(&(value->xint32)), 4 * 8, T32_PARAM_TYPE_UINT32);
		else
			res = T32_BundledAccess(0, Handle, size * 8, outbits, (uint8_t*)(void*)(&(value->xint32)), 4 * 8, T32_PARAM_TYPE_UINT32);
	} else
	{
		size = lenbytes < 8 ? 8 : lenbytes;
		if (Handle > T32_TAPACCESS_HOLD)
			res = T32_BundledAccessStore(0, Handle, size * 8, outbits, (uint8_t*)(void*)(&(value->xint64)), 8 * 8, T32_PARAM_TYPE_UINT64);
		else
			res = T32_BundledAccess(0, Handle, size * 8, outbits, (uint8_t*)(void*)(&(value->xint64)), 8 * 8, T32_PARAM_TYPE_UINT64);
	}
	...
	int nInfoID, T32_Param value)
{
...
#if defined(ENABLE_APILOG)
	if (info64) {
		T32_ApiLog(__func__, T32APILOG_FENTRY, "(void*) p%x, %d, %u, %d, %lu", Handle, nInstanceType, nInstance, nInfoID, value.xint64);
	} else {
		T32_ApiLog(__func__, T32APILOG_FENTRY, "(void*) p%x, %d, %u, %d, %u", Handle, nInstanceType, nInstance, nInfoID, value.xint32);
	}
#endif
	...
	if (!info64) {
		outbits[lenbytes++] = value.xint32 & 0xff;
		outbits[lenbytes++] = (value.xint32 >> 8) & 0xff;
		outbits[lenbytes++] = (value.xint32 >> 16) & 0xff;
		outbits[lenbytes++] = (value.xint32 >> 24) & 0xff;
	}
	else {
		outbits[lenbytes++] = value.xint64 & 0xff;
		outbits[lenbytes++] = (value.xint64 >> 8) & 0xff;
		outbits[lenbytes++] = (value.xint64 >> 16) & 0xff;
		outbits[lenbytes++] = (value.xint64 >> 24) & 0xff;
		outbits[lenbytes++] = (value.xint64 >> 32) & 0xff;
		outbits[lenbytes++] = (value.xint64 >> 40) & 0xff;
		outbits[lenbytes++] = (value.xint64 >> 48) & 0xff;
		outbits[lenbytes++] = (value.xint64 >> 56) & 0xff;
	}
